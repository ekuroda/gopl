...
func TestForEachNode(t *testing.T) {
	inReader := strings.NewReader(testHTML)
	inDoc, err := html.Parse(inReader)
	if err != nil {
		t.Fatal(err)
	}

	var docBuf bytes.Buffer
	html.Render(&docBuf, inDoc)
	//fmt.Println(string(docBuf.Bytes()))
	inDoc, err = html.Parse(&docBuf)
	if err != nil {
		t.Fatal(err)
	}

	var outBuf bytes.Buffer
	forEachNode(&outBuf, inDoc, startElement, endElement)
	outRendered := string(outBuf.Bytes())
	//fmt.Println(outRendered)
	outReader := strings.NewReader(outRendered)
	_, err = html.Parse(outReader)
	//outDoc, err := html.Parse(outReader)
	...
	// ダメ
	//visit(t, inDoc, outDoc)
}

func visit(t *testing.T, source *html.Node, target *html.Node) {
	//fmt.Printf("%v:%q:%v, %v:%q:%v\n", source.Type, source.Data, source.Attr, target.Type, target.Data, target.Attr)
	if source.Type == html.ElementNode || source.Type == html.CommentNode || source.Type == html.TextNode {
		if source.Type != target.Type {
			t.Errorf("target.Type = %v; want %v", target.Type, source.Type)
		}
		if source.Type == html.ElementNode {
			if source.Data != target.Data {
				t.Errorf("target.Data = %q; want %q", target.Data, source.Data)
			}
			if len(source.Attr) != len(target.Attr) {
				t.Errorf("len(target.Attr) = %d; want %d", len(target.Attr), len(source.Attr))
			} else {
				tattrs := target.Attr
				for i, sattr := range source.Attr {
					tattr := tattrs[i]
					if sattr.Key != tattr.Key || sattr.Val != tattr.Val {
						t.Errorf("target.Attr[%d] = %v; want %v", i, tattr, sattr)
					}
				}
			}
		}
	...
	sc, tc := source.FirstChild, target.FirstChild
	for {
		if sc == nil || sc.Type == html.ElementNode || sc.Type == html.CommentNode {
			break
		}
		if sc.Type == html.TextNode {
			if strings.TrimSpace(sc.Data) != "" {
				break
			}
		}
		sc = sc.NextSibling
	...
	for {
		if tc == nil || tc.Type == html.ElementNode || tc.Type == html.CommentNode {
			break
		}
	...
	for sc != nil || tc != nil {
		if sc == nil && tc != nil {
			t.Errorf("target = %v; want nil", tc)
			break
		}
		if sc != nil && tc == nil {
			t.Errorf("target = nil; want %v", sc)
			break
		}

		visit(t, sc, tc)

		sc, tc = sc.NextSibling, tc.NextSibling
		for {
			if sc == nil || sc.Type == html.ElementNode || sc.Type == html.CommentNode {
				break
			}
